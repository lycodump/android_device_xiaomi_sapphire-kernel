...
### Modules
# Cleanup / Preparation
rm -rf "${MY_DIR}/modules"
mkdir "${MY_DIR}/modules"

# Copy files to modules/${MODULE_FOLDER}/
for MODULE_FOLDER in "${MODULE_FOLDERS[@]}"; do
    mkdir -p "${MY_DIR}/modules/${MODULE_FOLDER}"
    ...
    if [ -d "${MODULE_SRC}" ]; then
        find "${MODULE_SRC}/lib/modules" -type f -exec cp {} "${MY_DIR}/modules/${MODULE_FOLDER}/" \; \
        -exec printf "  - modules/${MODULE_FOLDER}/{}\n" \;
    fi
done
